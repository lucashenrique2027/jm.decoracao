...
export default function DashboardBusiness() {
  ...
  useEffect(() => {
    async function carregarDashboard() {
      ...
      try {
        const [clientesData, produtosData, categoriasData] = await Promise.all([
          buscarFaturamentoClientes(periodo),
          buscarProdutosMaisVendidos(periodo),
          buscarCategoriasMaisVendidas(periodo),
        ]);
    ...
    carregarDashboard();
  }, [periodo]); // re-busca toda vez que o período mudar
  ...
  const faturamentoTotal = clientes.reduce((acc, c) => acc + Number(c.faturamento), 0);
  const totalPedidos     = clientes.reduce((acc, c) => acc + Number(c.totalPedidos), 0);
    ...
    <div className="container-fluid px-0">
          ...
          {PERIODOS.map(({ valor, label }) => (
            <button
              ...
              style={{
                ...
                fontWeight: 600,
                fontSize: '0.8rem',
                backgroundColor: periodo === valor ? '#0f172a' : '#f1f5f9',
                color:           periodo === valor ? '#fff'    : '#64748b',
                border: 'none',
              }}
            >
              {label}
      ...
      <div className="card border-0 shadow-sm mb-4" style={{ borderRadius: '16px', overflow: 'hidden' }}>
        <div className="card-body p-4">
          ...
          {produtos.length === 0 ? (
            <p className="text-muted text-center py-3 mb-0">Nenhuma venda no período selecionado.</p>
          ) : (
